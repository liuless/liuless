...
<div class="progress m-t-10" id="passwordStrengthBar">
    <div class="progress-bar" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100">
        <span class="sr-only">New Password Rating: 0%</span>
    </div>
</div>
<div class="alert alert-secondary m-t-10">
    <strong>{$LANG.strongpassword}</strong><br />{$LANG.casecharacters}<br />{$LANG.leastonesymbol}(# $ ! % &amp; etc...)<br />{$LANG.continuoustext}
</div>
<script type="text/javascript">
    jQuery("#inputNewPassword1").keyup(function() {
        ...
        jQuery("#passwordStrengthBar .progress-bar").removeClass("bg-danger bg-warning bg-success").css("width", pwstrength + "%").attr('aria-valuenow', pwstrength);
    ...
    function validatePassword2() {
        ...
        if (password2 && password1 !== password2) {
            ...
            jQuery("#inputNewPassword2Msg").html('<p class="help-block">{$LANG.passwordmismatch}</p>');
        } else {
            if (password2) {
